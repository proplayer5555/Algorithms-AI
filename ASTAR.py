...
# Add to closelist
# Calculate Total weight AND remove manhattan value
# Set current Cords to next node
# Check if goal is reached
#
def astar (FullArray,n,startID,max):
    openlist = []
    closelist = []
    totalweight=0
    currentR = 0
    ...
    while (not finish):
        ...
        if (currentR != 0 and  FullArray[currentR-1][currentC].Deleted == False and unvisited(FullArray[currentR-2][currentC],closelist,openlist)):
            ...
            temp.combinedweight = totalweight + FullArray[currentR-1][currentC].weight + temp.manhattan #adds weight of link
        ...
        if (currentC != 2 * n - 2 and  FullArray[currentR][currentC+1].Deleted == False and unvisited(FullArray[currentR][currentC+2],closelist,openlist)):
            ...
            temp.combinedweight = totalweight + FullArray[currentR][currentC+1].weight + temp.manhattan
            temp.parentID = FullArray[currentR][currentC].ID                                  #setup parent ID
            temp.depth = FullArray[currentR][currentC].depth + 1                              #setup depth in node addition
        ...
        totalweight = ( nextnode.combinedweight - nextnode.manhattan )
        #correct Node weight back to normal
        nextnode.combinedweight -= nextnode.manhattan
